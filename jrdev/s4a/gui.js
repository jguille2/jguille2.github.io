...
IDE_Morph.prototype.settingsMenu = function () {
    ...
    //adding extra s4a items for all versions

    // Jr mode switch
    menu.addItem(
        (this.isJr ? '\u2611 ' : '\u2610 ') + localize('Mode Jr.'),
        function () {
            this.isJr = !this.isJr;
            if (this.isJr) {
                this.startJrMode();
        ...
        this.isJr? 'uncheck to stop Jr mode' : 'check to start Jr mode'
    );

    menu.popup(this.world(), pos);
};

IDE_Morph.prototype.isJr = false;
...
IDE_Morph.prototype.projectMenu = function () {
    ...
    if (this.isJr) {
        menu.addItem(
            ...
            'newStoppingJr',
            'New standard Snap4Arduino project\nleaving icon-based blocks mode\n'
        );
    } else {
        menu.addItem(
            'New Jr. project', 
            'newStartingJr',
            'New Jr Snap4Arduino project.\nAn icon-based blocks mode\n'
...
IDE_Morph.prototype.JrPrimitivesIconization = function () {

    SpriteMorph.prototype.blocks.receiveGo.spec = "   %greenflag";
    SpriteMorph.prototype.blocks.receiveKey.spec = "$⌨-2 %keyHat";
    SpriteMorph.prototype.blocks.receiveInteraction.spec = "$🖱-2 %interaction";
    SpriteMorph.prototype.blocks.receiveMessage.spec = "$📬-2 %msgHat";
    SpriteMorph.prototype.blocks.doBroadcast.spec = "$📧-2 %msg";
    SpriteMorph.prototype.blocks.reportTouchingObject.spec = "$👉-2 %col ?";
    SpriteMorph.prototype.blocks.reportTouchingColor.spec = "$👉-2 %clr ?";
    SpriteMorph.prototype.blocks.setColor.spec = "$🎨-2 %clr";
    SpriteMorph.prototype.blocks.reportMouseDown.spec = "$🖱-2 ?";
    SpriteMorph.prototype.blocks.reportKeyPressed.spec = "$⌨-2 %key ?";
    SpriteMorph.prototype.blocks.reportAnalogReading.spec = "$👉🎚️-2 %analogPin ?";
    SpriteMorph.prototype.blocks.digitalWrite.spec = "$💡-2 %digitalPin %b";
    SpriteMorph.prototype.blocks.reportDigitalReading.spec = "$👉🔘-2 %digitalPin";

    this.flushBlocksCache();
    this.refreshPalette();
};

IDE_Morph.prototype.loadSnap4arduinoJrLib = function () {

    var myself = this;
    this.getURL(
        myself.resourceURL('libraries', 'snap4arduinoJr.xml'),
        function (txt) {
            myself.droppedText(txt, 'Snap4ArduinoJr');
        }
    );
};

IDE_Morph.prototype.hideNoJrPrimitives = function () {

    var hiddens = "reportColorIsTouchingColor doAsk getLastAnswer reportMouseX reportMouseY doResetTimer getTimer reportAttributeOf reportGet reportURL reportIsFastTracking doSetFastTracking reportDate reportDistanceTo doSetVar doChangeVar doShowVar doHideVar doDeleteAttr doDeclareVariables reportNewList reportCONS reportListItem reportCDR reportListLength reportListContainsItem doAddToList doDeleteFromList doInsertInList doReplaceInList reportJoinWords reportTextSplit reportLetter reportStringSize reportUnicode reportUnicodeAsLetter reportIsA reportIsIdentical reportJSFunction reportNot reportOr reportAnd reportRandom reportMonadic reportRound reportModulus reifyScript reifyPredicate reifyReporter doSwitchToCostume doWearNextCostume getCostumeIdx doSayFor bubble doThinkFor doThink changeEffect setEffect clearEffects changeScale setScale getScale show hide comeToFront goBack doScreenshot reportCostumes alert log forward turn turnLeft setHeading doFaceTowards gotoXY doGotoObject doGlide changeXPosition setXPosition changeYPosition setYPosition bounceOffEdge xPosition yPosition direction clear down up changeHue setHue changeBrightness setBrightness changeSize doStamp floodFill playSound doPlaySoundUntilDone doStopAllSounds doRest doPlayNote doChangeTempo doSetTempo getTempo reportSounds setSize doBroadcastAndWait getLastMessage doWarp doWait doWaitUntil doForever doRepeat doUntil doIf doIfElse doReport doStopThis doStopOthers doRun fork evaluate doCallCC reportCallCC receiveOnClone createClone removeClone doPauseAll connectArduino disconnectArduino servoWrite pwmWrite doTellTo reportAskFor newClone doSetInstrument reportPenTrailsAsCostume reportRelationTo reportConnected";
    hiddens = hiddens.split(' ');
    hiddens.forEach(function (sel) {
...
IDE_Morph.prototype.dressWithJrTortugaCostume = function () {

    var myself = this,
        url = this.resourceURL('Costumes', 'tortugaJr.png'),
        img = new Image(),
    ...
    cstTemplate = icon = new CostumeIconMorph(
        new Costume(turtle.image, 'tortuga'),
        cstTemplate
    );
    ...
    img.onload = function () {
        ...
        icon.object = new Costume(canvas, 'tortuga');
        icon.refresh();
        myself.droppedImage(
...
IDE_Morph.prototype.paintJrPentrails = function () {
    var myself = this,
        url = this.resourceURL('Backgrounds', 'backgroundJr.png'),
        img = new Image(),
        turtle = new SymbolMorph('turtle', 60),
        icon,
        cstTemplate;
    ...
    cstTemplate = icon = new CostumeIconMorph(
        new Costume(turtle.image, 'backgroundJr'),
        cstTemplate
    );
    ...
    img.onload = function () {
        var canvas = newCanvas(new Point(img.width, img.height), true);
        canvas.getContext('2d').drawImage(img, 0, 0);
        icon.object = new Costume(canvas, 'backgroundJr');
        icon.refresh();
        myself.stage.trailsCanvas = icon.object.contents;
    };

    img.src = url;
};

IDE_Morph.prototype.JrPrimitivesNormalization = function () {
    SpriteMorph.prototype.blocks.receiveGo.spec = 'when %greenflag clicked';
    SpriteMorph.prototype.blocks.receiveKey.spec = 'when %keyHat key pressed';
    SpriteMorph.prototype.blocks.receiveInteraction.spec = 'when I am %interaction';
    SpriteMorph.prototype.blocks.receiveMessage.spec = 'when I receive %msgHat';
    SpriteMorph.prototype.blocks.doBroadcast.spec = 'broadcast %msg';
    SpriteMorph.prototype.blocks.reportTouchingObject.spec = 'touching %col ?';
    SpriteMorph.prototype.blocks.reportTouchingColor.spec = 'touching %clr ?';
    SpriteMorph.prototype.blocks.setColor.spec = 'set pen color to %clr';
    SpriteMorph.prototype.blocks.reportMouseDown.spec = 'mouse down?';
    SpriteMorph.prototype.blocks.reportKeyPressed.spec = 'key %key pressed?';
    SpriteMorph.prototype.blocks.reportAnalogReading.spec = 'analog reading %analogPin';
    SpriteMorph.prototype.blocks.digitalWrite.spec = 'set digital pin %digitalPin to %b';
    SpriteMorph.prototype.blocks.reportDigitalReading.spec = 'digital reading %digitalPin';

    this.flushBlocksCache();
    this.refreshPalette();
};

IDE_Morph.prototype.showAllPrimitives = function () {

    var hiddens = StageMorph.prototype.hiddenPrimitives;
    Object.keys(hiddens).forEach(function (block) {
        delete StageMorph.prototype.hiddenPrimitives[block];
    });
...
IDE_Morph.prototype.newStartingJr = function () {

    this.isJr = true;
    this.createNewProject();
};

IDE_Morph.prototype.newStoppingJr = function () {

    this.isJr = false;
    this.JrPrimitivesNormalization();
    this.createNewProject();
};

IDE_Morph.prototype.startJrMode = function () {

    this.isJr = true;
    this.JrPrimitivesIconization();
    this.loadSnap4arduinoJrLib();
};

IDE_Morph.prototype.stopJrMode = function () {
...
IDE_Morph.prototype.createNewProject = function () {
    ...
    this.confirm(
            ...
            function () {
                ...
                myself.newProject();
                if (myself.isJr) {
                    myself.JrPrimitivesIconization();
                    myself.loadSnap4arduinoJrLib();
                    myself.hideNoJrPrimitives();
                    myself.dressWithJrTortugaCostume();
                    myself.paintJrPentrails();
                }
            }
            );
};
